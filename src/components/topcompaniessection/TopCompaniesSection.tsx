import topCompany from 'common/data/topCompany';
import { Link } from 'react-router-dom';
import Slider from 'react-slick';
const TopCompaniesSection = () => {
  const topCompanySettings = {
    dots: true,
    speed: 500,
    slidesToShow: 4,
    slidesToScroll: 4,
      ...
      <div className="auto-container">
        ...
        <div className="carousel-outer" data-aos="fade-up">
          <div className="companies-carousel">
            {/* <TopCompany /> */}
            <Slider {...topCompanySettings} arrows={false}>
              {topCompany.slice(0, 12).map((company: any) => (
                <div className="company-block" key={company.id}>
                  <div className="inner-box">
                    ...
                    <h4 className="name">
                      <Link to={`employers-single-v1/${company.id}`}>{company.name}</Link>
                    </h4>
                    <div className="location">
                      <i className="flaticon-map-locator"></i> {company.location}
                    </div>
                    <Link to={`employers-single-v1/${company.id}`} className="theme-btn btn-style-three">
