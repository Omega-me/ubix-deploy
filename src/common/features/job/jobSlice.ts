...
export const jobSlice = createSlice({
    ...
    initialState,
    reducers: {
        addLatestJob: (state: any, { payload }) => {
        ...
        clearJobTypeToggle: (state: any) => {
            state?.jobTypeList?.map((item: any) => {
        ...
        jobTypeCheck: (state: any, { payload }) => {
            state?.jobTypeList?.map((item: any) => {
                if (item.id === payload) {
                    if (item.isChecked) {
                        item.isChecked = false;
        ...
        datePostCheck: (state: any, { payload }) => {
            state?.datePost?.map((item: any) => {
                item.isChecked = false;
                if (item.id === payload) {
                    item.isChecked = true;
                }
        ...
        experienceLavelCheck: (state: any, { payload }) => {
            state?.experienceLavel?.map((item: any) => {
                if (item.id === payload) {
                    if (item.isChecked) {
                        item.isChecked = false;
                    } else {
        ...
        clearExperienceToggle: (state: any) => {
            state?.experienceLavel?.map((item: any) => {
                item.isChecked = false;
                return {
                    ...item,
