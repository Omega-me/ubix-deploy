 ...
 * @returns
 */
export const useExperiencesQuery = <TData = any>(config?: IQueryConfig<TData>) => {
    const auth = useAuth<{ token: string; }>();
    const queryClient = useQueryClient();

    let hasAuthentication = true;
    let url: string = eApiRoutes.EXPERIENCES;
    let keys: QueryKey = [eQueryKeys.EXPERIENCES];

    if (config?.queryConfig?.queryUrl) {
        url = `${eApiRoutes.EXPERIENCES}/${config.queryConfig.queryUrl}`;
        keys = [...keys, config.queryConfig.queryUrl];
    }
    if (config?.queryConfig?.queryParam) {
        url = `${eApiRoutes.EXPERIENCES}/${config.queryConfig.queryParam}`;
    ...
    if (config?.queryConfig?.queryString) {
        ...
        url = `${eApiRoutes.EXPERIENCES}?${config.queryConfig.queryString}`;
    }
    ...
    const { data, ...query } = useQuery(
        ...
        {
            onSuccess(data) {
                queryClient.invalidateQueries(eApiRoutes.EXPERIENCES);
 ...
 * @returns
 */
export const useExperiencesMutation = <TData = unknown>(config?: IMuattionConfig<TData>) => {
