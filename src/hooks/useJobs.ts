 ...
 * @returns
 */
export const useJobsQuery = <TData = any>(config?: IQueryConfig<TData>) => {
    const auth = useAuth<{ token: string; }>();
    const queryClient = useQueryClient();

    let hasAuthentication = true;
    let url: string = eApiRoutes.JOBS;
    let keys: QueryKey = [eQueryKeys.JOBS];

    if (config?.queryConfig?.queryUrl) {
        url = `${eApiRoutes.JOBS}/${config.queryConfig.queryUrl}`;
        keys = [...keys, config.queryConfig.queryUrl];
    }
    if (config?.queryConfig?.queryParam) {
        url = `${eApiRoutes.JOBS}/${config.queryConfig.queryParam}`;
    ...
    if (config?.queryConfig?.queryString) {
        ...
        url = `${eApiRoutes.JOBS}?${config.queryConfig.queryString}`;
    }
    ...
    const { data, ...query } = useQuery(
        ...
        {
            onSuccess(data) {
                queryClient.invalidateQueries(eApiRoutes.JOBS);
 ...
 * @returns
 */
export const useJobsMutation = <TData = unknown>(config?: IMuattionConfig<TData>) => {
